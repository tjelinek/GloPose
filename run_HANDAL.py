    ...
    for obj_type_sequence in sequences:
        with exception_logger(obj_type_sequence):
            ...
            if output_folder is not None:
                write_folder = Path(output_folder) / dataset / f'{config.run.special_hash}_{sequence}'
            else:
                write_folder = config.paths.results_folder / experiment_name / dataset / \
                               f'{config.run.special_hash}_{sequence}'
