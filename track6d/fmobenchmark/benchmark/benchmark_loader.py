	...
	for kkf, ff in enumerate(files):
		...
		for kk in range(gtp.nfrms):
			...
			if args.add_traj:
				...
				est_hs, est_traj, model_val = method(I,B,bbox_tight,gtp.nsplits,radius,obj_dim)
			...
			psnr_values.append(seq_score_tracker.all_psnr[kk])
			ssim_values.append(seq_score_tracker.all_ssim[kk])
