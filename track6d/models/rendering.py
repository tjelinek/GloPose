...
class RenderingKaolin(nn.Module):
    ...
    def forward(self, translation, quaternion, unit_vertices, face_features, texture_maps=None, lights=None, render_depth=False):
        ...
        for frmi in range(quaternion.shape[1]):
            ...
            if not lights is None:
                ...
                for li in range(lights.shape[0]):
                    lighting_r = kaolin.render.mesh.utils.spherical_harmonic_lighting(im_normals, lights[li:(li+1),0])[...,None]
                    lighting_g = kaolin.render.mesh.utils.spherical_harmonic_lighting(im_normals, lights[li:(li+1),1])[...,None]
                    lighting_b = kaolin.render.mesh.utils.spherical_harmonic_lighting(im_normals, lights[li:(li+1),2])[...,None]
