...
from fmobenchmark.benchmark.loaders_helpers import get_falling_dataset, get_tbd3d_dataset, get_tbd_dataset
...
def get_figure_images(encoder, rendering, device, mode, seqi, frmi, results_mode=False, n_occ=2):
    log_folder = tmp_folder + 'paper_imgs/'
    ...
    for kkf, ff in enumerate(files):
        ...
        for kk in range(gtp.nfrms):
            ...
            if n_occ > 9:
                ...
                ext2 = ((bbox_tight[2] - bbox_tight[0]) - (bbox_tight[3] - bbox_tight[1])) // 2 + (
                        bbox_tight[2] - bbox_tight[0]) // 4
                if ext2 > 0:
            ...
            if do_deblatting or do_tbdo:
                if gtp.syn_mode:
                    bbox_temp = bbox_detect_hs(crop_only(gt_hs[:, :, [3, 3, 3], -1], bbox_tight),
                                               crop_only(np.zeros(B.shape), bbox_tight))
                ...
                if do_tbdo and gtp.w_trajgt:
                    rgba_tbd3d_or, Hso_crop = deblatting_oracle_runner(crop_only(I, bbox_debl), crop_only(B, bbox_debl),
                                                                       debl_dim, gt_traj[[1, 0]] - bbox_debl[:2, None])
                    Hso = rev_crop_resize(Hso_crop[:, :, None, :][:, :, [-1, -1, -1], :], bbox_debl, np.zeros(I.shape))

                if do_deblatting:
                    est_hs_tbd_crop, est_hs_tbd3d_crop, rgba_tbd, rgba_tbd3d, est_traj_tbd, Hs_crop = deblatting_runner(
                        crop_only(I, bbox_debl), crop_only(B, bbox_debl), gtp.nsplits, debl_dim)
            ...
            if do_deblatting:  # and gtp.use_hs
                seq_score_tracker_tbd.next_appearance(kk, crop_only(gt_hs[:, :, :3], bbox_tight),
                                                      crop_only(est_hs_tbd, bbox_tight), crop_only(I, bbox_tight),
                                                      crop_only(B, bbox_tight))
                seq_score_tracker_tbd3d.next_appearance(kk, crop_only(gt_hs[:, :, :3], bbox_tight),
                                                        crop_only(est_hs_tbd3d, bbox_tight), crop_only(I, bbox_tight),
                                                        crop_only(B, bbox_tight))
            ...
            for tempi in offsets:
                ...
                for ind0 in inds:
                    ...
                    mask = binary_dilation(renders_rgba_full[:, :, -1, ind0] > 0.001, iterations=10)[:, :, None][:, :,
                           [0, 0, 0]]
                    if gtp.syn_mode:
                    ...
                    if do_deblatting:
                        ...
                        renders_a_joint_tbd = fmo_model(renders_a_joint_tbd, Hsc, rgba_tbd[:, :, [3, 3, 3], ind0],
                                                        rgba_tbd[:, :, 3, ind0])
                        est_hs_joint_tbd = fmo_model(est_hs_joint_tbd, Hsc, rgba_tbd[:, :, :3, ind0],
                                                     rgba_tbd[:, :, 3, ind0])
                        renders_rgba_joint_tbd = fmo_model(renders_rgba_joint_tbd, Hsc, rgba_tbd[:, :, :3, ind0],
                                                           rgba_tbd[:, :, 3, ind0])

                        renders_a_joint_tbd3d = fmo_model(renders_a_joint_tbd3d, Hsc, rgba_tbd3d[:, :, [3, 3, 3], ind0],
                                                          rgba_tbd3d[:, :, 3, ind0])
                        est_hs_joint_tbd3d = fmo_model(est_hs_joint_tbd3d, Hsc, rgba_tbd3d[:, :, :3, ind0],
                                                       rgba_tbd3d[:, :, 3, ind0])
                        renders_rgba_joint_tbd3d = fmo_model(renders_rgba_joint_tbd3d, Hsc, rgba_tbd3d[:, :, :3, ind0],
                                                             rgba_tbd3d[:, :, 3, ind0])
                    if do_tbdo:
                        Hsco = Hso[:, :, 0, ind0] / np.sum(Hso[:, :, 0, ind0])
                        renders_a_joint_tbd3do = fmo_model(renders_a_joint_tbd3do, Hsco,
                                                           rgba_tbd3d_or[:, :, [3, 3, 3], ind0],
                                                           rgba_tbd3d_or[:, :, 3, ind0])
                        est_hs_joint_tbd3do = fmo_model(est_hs_joint_tbd3do, Hsco, rgba_tbd3d_or[:, :, :3, ind0],
                                                        rgba_tbd3d_or[:, :, 3, ind0])
                        renders_rgba_joint_tbd3do = fmo_model(renders_rgba_joint_tbd3do, Hsco,
                                                              rgba_tbd3d_or[:, :, :3, ind0],
                                                              rgba_tbd3d_or[:, :, 3, ind0])

                est_hs_joint[est_hs_joint < 0] = 0
                renders_rgba_joint[renders_rgba_joint < 0] = 0
                imwrite(crop_only(1 - renders_a_joint[..., [-1, -1, -1]], bbox_tight),
                        log_folder + gtp.seqname + '_{:04d}_estm{}.png'.format(kk, tempi))
                imwrite(crop_only(renders_rgba_joint, bbox_tight) ** gamma_c,
                        log_folder + gtp.seqname + '_{:04d}_estfm{}.png'.format(kk, tempi))
                imwrite(crop_only(est_hs_joint, bbox_tight) ** gamma_c,
                        log_folder + gtp.seqname + '_{:04d}_est{}.png'.format(kk, tempi))
                if do_deblatting:
                    ...
                    imwrite(crop_only(1 - renders_a_joint_tbd[..., [-1, -1, -1]], bbox_tight),
                            log_folder + gtp.seqname + '_{:04d}_tbdm{}.png'.format(kk, tempi))
                    imwrite(crop_only(1 - renders_a_joint_tbd3d[..., [-1, -1, -1]], bbox_tight),
                            log_folder + gtp.seqname + '_{:04d}_tbd3dm{}.png'.format(kk, tempi))
                    imwrite(crop_only(renders_rgba_joint_tbd, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbdfm{}.png'.format(kk, tempi))
                    imwrite(crop_only(renders_rgba_joint_tbd3d, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbd3dfm{}.png'.format(kk, tempi))
                    imwrite(crop_only(est_hs_joint_tbd, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbd{}.png'.format(kk, tempi))
                    imwrite(crop_only(est_hs_joint_tbd3d, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbd3d{}.png'.format(kk, tempi))

                if gtp.syn_mode:
                    imwrite(crop_only(1 - gt_hs_a[..., [-1, -1, -1]], bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_hsm{}.png'.format(kk, tempi))
                    imwrite(crop_only(gt_hs_sum, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_hs{}.png'.format(kk, tempi))
                elif gtp.use_hs:
                    gt_hs_a = np.sum(np.abs(gt_hs - B[..., None]), 2)[:, :, None, :].astype(float) * 8
                    imwrite(crop_only(1 - gt_hs_a[..., [-1, -1, -1], inds[0]], bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_hsm{}.png'.format(kk, tempi))
                    imwrite(crop_only(gt_hs_sum, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_hs{}.png'.format(kk, tempi))
                ...
                if do_tbdo:
                    ...
                    imwrite(crop_only(1 - renders_a_joint_tbd3do[..., [-1, -1, -1]], bbox_tight),
                            log_folder + gtp.seqname + '_{:04d}_tbd3dom{}.png'.format(kk, tempi))
                    imwrite(crop_only(renders_rgba_joint_tbd3do, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbd3dofm{}.png'.format(kk, tempi))
                    imwrite(crop_only(est_hs_joint_tbd3do, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_tbd3do{}.png'.format(kk, tempi))

                if do_sota18:
                    imwrite(crop_only(est_hs_sota18[..., 0], bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_sota18_{}.png'.format(kk, 0))
                    imwrite(crop_only(est_hs_sota18[..., -1], bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_sota18_{}.png'.format(kk, 7))

                if do_deblurgan:
                    # imwrite(crop_only(est_hs_deblurgan,bbox_tight)**gamma_c,log_folder+gtp.seqname+'_{:04d}_deblurgan.png'.format(kk))
                    imwrite(crop_only(est_hs_deblurgan_hr, bbox_tight) ** gamma_c,
                            log_folder + gtp.seqname + '_{:04d}_deblurgan_hr.png'.format(kk))
