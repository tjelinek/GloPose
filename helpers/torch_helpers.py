...
def renders2traj(renders, device):
    ...
    cenx = torch.sum(sumy * torch.arange(1, sumy.shape[-1] + 1)[None, None].float().to(device), -1) / torch.sum(sumy,
                                                                                                                -1)
    ceny = torch.sum(sumx * torch.arange(1, sumx.shape[-1] + 1)[None, None].float().to(device), -1) / torch.sum(sumx,
                                                                                                                -1)
...
def write_latent(rendering, latent, device, folder, steps, frmi=0, videoname=None):
    ...
    with torch.no_grad():
        ...
        for ki in range(renders.shape[2]):
            ...
            if write_video:
                if out is None:
                    out = cv2.VideoWriter(os.path.join(folder, videoname), cv2.VideoWriter_fourcc(*"MJPG"), 6,
                                          (renders.shape[5], renders.shape[4]), True)
