...
class Tracking6D:
    def __init__(self, config, device, write_folder, file0, bbox0, init_mask=None):
        ...
        width = shape[-1]
        height = shape[-2]
        self.rendering = RenderingKaolin(self.config, self.faces, width, height).to(self.device)
    ...
    def run_tracking(self, files, bboxes, depths_dict=None):
        ...
        for stepi in range(1, self.config["input_frames"]):
            ...
            self.images = torch.cat((self.images, image), 1)  # images x h x w, self.images[-1] is the last one observed
            image_feat = self.feat(image).detach()
            self.images_feat = torch.cat((self.images_feat, image_feat), 1)
            self.segments = torch.cat((self.segments, segment), 1)  # segmentation mask
            self.keyframes.append(stepi)
            start = time.time()
            b0 = get_bbox(self.segments)

            self.rendering_old = self.rendering.copy()  # Keep the old rendering to compute flow los
            width = b0[3] - b0[2]
            height = b0[1] - b0[0]
            self.rendering = RenderingKaolin(self.config, self.faces, width, height).to(self.device)
            # TODO get the rendering mesh

            flow_rendering = get_flow_from_images(self.rendering, self.rendering_old)
            # TODO rendering flow to loss function

            self.encoder.offsets[:, :, stepi, :3] = (self.encoder.used_tran[:, :, stepi - 1] +
                                                     self.encoder.offsets[:, :, stepi - 1, :3])

            self.encoder.offsets[:, 0, stepi, 3:] = qmult(qnorm(self.encoder.used_quat[:, stepi - 1]),
                                                          qnorm(self.encoder.offsets[:, 0, stepi - 1, 3:]))

            # Actual loss observed between the two video frames
            flow_observed = None   # Todo find what are the rgb video frame
