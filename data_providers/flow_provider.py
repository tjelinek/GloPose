...
class FlowCache:
    """Handles disk and datagraph caching for flow providers."""

    def __init__(self, cache_dir: Path, data_graph: DataGraph = None,
                 allow_disk_cache: bool = False, purge_cache: bool = False):
    ...
    def try_load_flow(self, device: str, source_image_index, target_image_index,
                      source_image_name, target_image_name):
        """Try loading flow from datagraph or disk. Returns (warp, certainty) or (None, None)."""
        certainty_filename, warp_filename = self._get_cache_filenames(
        ...
        if (warp is None or certainty is None) and warp_filename is not None and certainty_filename is not None:
            if warp_filename.exists() and certainty_filename.exists() and self.allow_disk_cache:
                warp = torch.load(warp_filename, weights_only=True, map_location=device)
                certainty = torch.load(certainty_filename, weights_only=True, map_location=device)

        return warp, certainty

    def save_flow_to_disk(self, warp, certainty, source_image_name, target_image_name):
        if source_image_name and target_image_name and self.allow_disk_cache:
            saved_filename = f'{source_image_name.stem}___{target_image_name.stem}.pt'
            torch.save(warp, self.warps_path / saved_filename)
            torch.save(certainty, self.certainties_path / saved_filename)
    ...
    def try_load_points(self, source_image_index, target_image_index):
        """Try loading cached source/target points from datagraph. Returns (src_pts, dst_pts) or (None, None)."""
        if self._datagraph_edge_exists(source_image_index, target_image_index):
            edge_data = self.data_graph.get_edge_observations(source_image_index, target_image_index)
            if edge_data.src_pts_xy_roma is not None and edge_data.dst_pts_xy_roma is not None:
                return edge_data.src_pts_xy_roma, edge_data.dst_pts_xy_roma
        return None, None

    def save_points_to_datagraph(self, source_image_index, target_image_index, src_pts_xy, dst_pts_xy, certainty):
    ...
    def cached_flow_from_filenames(self, device: str, src_image_name: Union[str, Path],
                                   target_image_name: Union[str, Path]):
        ...
        if warp_filename.exists() and certainty_filename.exists():
            warp = torch.load(warp_filename, weights_only=True).to(device)
            certainty = torch.load(certainty_filename, weights_only=True).to(device)
            return warp, certainty
...
class FlowProviderDirect(ABC):
    ...
    def __init__(self, device: str, cache: FlowCache = None):
        self.device = device
        self.cache: Optional[FlowCache] = cache
    ...
    def get_source_target_points(self, source_image: torch.Tensor, target_image: torch.Tensor,
        ...
        # Try loading cached points from datagraph
        if self.cache is not None:
            src_pts_xy, dst_pts_xy = self.cache.try_load_points(source_image_index, target_image_index)
...
class RoMaFlowProviderDirect(FlowProviderDirect):
    ...
    def _compute_flow_raw(self, source_image_tensor: torch.Tensor,
    ...
    def compute_flow(self, source_image_tensor: torch.Tensor, target_image_tensor: torch.Tensor, sample=None,
        ...
        if self.cache is not None:
            warp, certainty = self.cache.try_load_flow(
                self.device, source_image_index, target_image_index, source_image_name, target_image_name)

        if warp is None or certainty is None:
            warp, certainty = self._compute_flow_raw(source_image_tensor, target_image_tensor)

            if self.cache is not None:
                self.cache.save_flow_to_disk(warp, certainty, source_image_name, target_image_name)
