...
def calculate_ssim(img1, img2):
    ...
    if len(img1.shape) == 3:
        ...
        for kk in range(ssims.shape[0]):
            if len(img2.shape) == 4:
                ssims[kk] = metrics.structural_similarity(img1[:, :, :, kk], img2[:, :, :, kk],
                                                          data_range=img2.max() - img2.min(), multichannel=True)
            else:
                ssims[kk] = metrics.structural_similarity(img1[:, :, :, kk], img2, data_range=img2.max() - img2.min(),
                                                          multichannel=True)
...
def rev_crop_resize(inp, bbox, I):
    ...
    for hsk in range(inp.shape[3]):
        est_hs[bbox[0]:bbox[2], bbox[1]:bbox[3], :, hsk] = cv2.resize(inp[:, :, :, hsk],
...
def sync_directions(est_hs_crop, gt_hs_crop):
    ...
    if gt_hs_crop is not None:
        if np.mean((est_hs_crop[:, :, :, 0] - gt_hs_crop[:, :, :, 0]) ** 2) > np.mean(
                (est_hs_crop[:, :, :, 0] - gt_hs_crop[:, :, :, -1]) ** 2):
