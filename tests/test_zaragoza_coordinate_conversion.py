...
from utils.math_utils import Se3_obj_from_epipolar_Se3_cam
...
from pose.epipolar_pose_estimator import recover_scale
...
def predict_camera_pose_using_zaragoza(source_frame_, target_frame_, config_, rendering_):
    ...
    quat_cam_ = Quaternion.from_axis_angle(rot_cam_)
    Se3_cam_ = Se3(quat_cam_, t_cam_.squeeze(-1))
...
Se3_world_to_cam = rendering.camera_transformation_matrix_Se3()
...
Se3_cam_ref_to_last = recover_scale(Se3_cam_ref_to_last, Se3_world_to_cam)
Se3_cam_first_to_ref = recover_scale(Se3_cam_first_to_ref, Se3_world_to_cam)
Se3_cam_first_to_last = recover_scale(Se3_cam_first_to_last, Se3_world_to_cam)

Se3_obj_ref_to_last_pred = Se3_obj_from_epipolar_Se3_cam(Se3_cam_ref_to_last, Se3_world_to_cam)
...
print(f'T obj ref to last pred: {Se3_obj_ref_to_last_pred.translation.numpy(force=True).round(3) / Se3_obj_ref_to_last_gt.translation.numpy(force=True)}')
print(f'T obj ref to last   gt: {Se3_obj_ref_to_last_gt.translation.numpy(force=True).round(3)}')
print(f'T obj 1st to last pred: {Se3_obj_1st_to_last_pred.translation.numpy(force=True).round(3) / Se3_obj_1st_to_last_gt.translation.numpy(force=True)}')
