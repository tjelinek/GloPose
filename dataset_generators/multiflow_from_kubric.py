...
sys.path.append('MFT')
from MFT.utils.io import write_flowou_X16
...
def write_outputs(outputs, directory, frames):
    flows_absolute = outputs['flow']
    occlusions = outputs['occlusion']
    for frame in frames:
        flow_absolute = flows_absolute[frame]
        occlusion = occlusions[frame]
        uncertainty = np.zeros(occlusion.shape)

        output_file = Path(directory) / f"flow_absolute_{frame:05d}.pkl"
        write_flowou_X16(output_file, flow_absolute, occlusion, uncertainty)


def count_forward_flow_files(directory_path):
    try:
        # Get a list of all files in the specified directory
        files = os.listdir(directory_path)

        # Use a list comprehension to count files with names starting with "forward_flow"
        count = sum(1 for file in files if file.startswith("forward_flow"))

        return count
    except OSError as e:
        # Handle any potential errors (e.g., directory not found)
        print(f"Error: {e}")
        return -1  # Return -1 to indicate an error


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process a folder and run some functions.')
    parser.add_argument('--folder', type=str, help='Path to the folder to operate on')

    args = parser.parse_args()
    folder_path = args.folder

    frames = list(range(count_forward_flow_files(folder_path)))
    flow_outputs = get_multiflow(folder_path, frames)
    write_outputs(flow_outputs, folder_path, frames)
