...
def average_patch_similarity_np(
        patch_descriptors1,
        patch_descriptors2,
        segmentation_mask1=None,
        segmentation_mask2=None,
        use_segmentation=False,
        required_share_nonzero=0.25,
):
    B, P, _, D = patch_descriptors1.shape
    ...
    if use_segmentation:
        mask1 = _create_patch_mask_np(segmentation_mask1, P, required_share_nonzero)
        mask2 = _create_patch_mask_np(segmentation_mask2, P, required_share_nonzero)
    else:
        mask1 = np.ones((B, P * P), dtype=bool)
        mask2 = np.ones((B, P * P), dtype=bool)

    sim_1to2 = _compute_nearest_neighbor_similarity_np(desc1_norm, desc2_norm, mask1, mask2)
    sim_2to1 = _compute_nearest_neighbor_similarity_np(desc2_norm, desc1_norm, mask2, mask1)
...
def _create_patch_mask_np(segmentation_mask, P, required_share_nonzero):
    B, H, W = segmentation_mask.shape
    patch_h = H // P
    patch_w = W // P

    mask_patches = segmentation_mask[:, :P * patch_h, :P * patch_w].reshape(B, P, patch_h, P, patch_w)
    mask_patches = mask_patches.transpose(0, 1, 3, 2, 4).reshape(B, P * P, patch_h * patch_w)

    nonzero_share = (mask_patches != 0).astype(float).mean(axis=-1)
    patch_mask = nonzero_share >= required_share_nonzero

    return patch_mask


def _compute_nearest_neighbor_similarity_np(desc_src, desc_tgt, mask_src, mask_tgt):
    B, N_src, D = desc_src.shape
    ...
    for b in range(B):
        ...
        cosine_sim = np.dot(src_valid, tgt_valid.T)
